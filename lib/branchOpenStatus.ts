...
export function getArgentinaNowParts(now: Date = new Date()) {
...
function isWithinMinuteRange(currentMinutes: number, openMinutes: number, closeMinutes: number) {
  // Overnight range that crosses midnight.
...
/** Pure open check for a given Argentina weekday + minutes-since-midnight. */
...
export function isBranchOpenNow(openingHours: string, now: Date = new Date()): boolean {
  const { day, minutes } = getArgentinaNowParts(now);
  return isBranchOpenAt(openingHours, day, minutes);
}

export function getBranchClosedCustomerMessage(openingHours: string): string {
  return `El local está cerrado. Podés armar el pedido y enviarlo al abrir. Horario: ${openingHours}`;
}
