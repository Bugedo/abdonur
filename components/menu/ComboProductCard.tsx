...
import { stripPresentationFromFlavorName } from '@/lib/empanadaMenu';
...
export function ComboProductCardInner({ product, flavorProducts, cartActions }: ComboProductCardInnerProps) {
  ...
  function addComboToCart() {
    ...
    const detail = flavorProducts
      ...
      .map((entry) => `${entry.qty}x ${stripPresentationFromFlavorName(entry.name)}`)
      .join(', ');
          ...
          <div className="rounded-lg border border-metallic-500/30 bg-brand-900/20 p-3 shadow-inner shadow-black/25 backdrop-blur-sm">
            ...
            <div className="mt-3 space-y-2">
              {flavorProducts.map((flavor) => (
                ...
                >
                  <span className="text-sm text-stone-200">{stripPresentationFromFlavorName(flavor.name)}</span>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
